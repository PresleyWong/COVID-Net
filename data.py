...
ref_cxr_img = cv2.imread("ref1.png", cv2.IMREAD_GRAYSCALE)
...
def match_histograms(src_image, ref_image):
    src = src_image
    ref = ref_image
    src_hist_blue, bin_0 = np.histogram(src.flatten(), 256, [0, 256])
    ref_hist_blue, bin_3 = np.histogram(ref.flatten(), 256, [0, 256])
    src_cdf_blue = calculate_cdf(src_hist_blue)
    ref_cdf_blue = calculate_cdf(ref_hist_blue)
    blue_lookup_table = calculate_lookup(src_cdf_blue, ref_cdf_blue)
    image_after_matching = cv2.LUT(src, blue_lookup_table)
    image_after_matching = cv2.convertScaleAbs(image_after_matching)
    return image_after_matching

def process_hist_match(src_image, rgb_flag=False):
    gray_img = cv2.cvtColor(src_image, cv2.COLOR_BGR2GRAY)
    match_img = match_histograms(gray_img, ref_cxr_img)
    if rgb_flag:
        output = cv2.cvtColor(match_img, cv2.COLOR_GRAY2RGB)
    else:
        output = cv2.cvtColor(match_img, cv2.COLOR_GRAY2BGR)
    return output
